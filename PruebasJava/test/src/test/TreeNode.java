 ...
 * a value,
 * together with a list of references to nodes (the children), with the
 * constraints that
 ...
 * <b>empty</b> tree. A tree that is not empty consists of a root node and
 * potentially many
 ...
 * <li><b>Parent</b> - a node, other than the root, which is connected to other
 * successor
 * nodes</li>
 * <li><b>Child</b> - a node, other than the root, which is connected to
 * predecessor</li>
 ...
 * <li><b>Subtree</b> - a node in a tree with all of its proper descendants, if
 * any</li>
 * <li><b>Node Height</b> - the number of edges on the longest downward path
 * between that
 * node and a leaf</li>
 * <li><b>Tree Height</b> - the number of edges on the longest downward path
 * between the
 * root and a leaf (root height)</li>
 * <li><b>Depth (Level)</b> - the path length between the root and the current
 * node</li>
 * <li><b>Ordered Tree</b> - tree in which nodes has the children ordered</li>
 * <li><b>Labeled Tree</b> - tree in which a label or value is associated with
 * each node
 * of the tree</li>
 * <li><b>Expression Tree</b> - tree which specifies the association of an
 * expression�s
 ...
 * <li><b>Pre order</b> - a form of tree traversal, where the action is called
 * firstly on
 * the current node, and then the pre order function is called again
 * recursively on each of the subtree from left to right</li>
 * <li><b>Post order</b> - a form of tree traversal, where the post order
 * function is called
...
public abstract class TreeNode<T> implements Iterable<TreeNode<T>>, Serializable, Cloneable {
	...
	/**
	 * Reference to the parent tree node. Is {@code null} if the current tree node
	 * is root
	 */
	...
	public Collection<? extends TreeNode<T>> findAll(final T data) {
		if (isLeaf()) {
			return (data() == null ? data == null : data().equals(data)) ? Collections.singleton(this)
					: Collections.<TreeNode<T>>emptySet();
		}
	 ...
	 * @return collection of nodes, which connect the current node with its
	 *         descendants
	 * @throws TreeNodeException exception that may be thrown in case if the
	 *                           current node does not have such descendant or if
	 *                           the
	 ...
	 * @throws TreeNodeException exception that may be thrown in case if the
	 *                           specified tree node is null or the specified tree
	 *                           node
	 *                           does not belong to the current tree or if any of
	 *                           the tree
	 *                           nodes either the current one or the specified one
	 *                           is root
	 */
	...
	/**
	 * Returns the number of nodes in the entire tree, including the current tree
	 * node
	 *
	 ...
	 * @return traversal action, which populates the input collection with the tree
	 *         nodes
	 */
	 ...
	 *
	 * @param node tree node to remove the parent tree node reference assignment
	 *             from
